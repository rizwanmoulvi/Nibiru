 ...
 * */
const bytesToHex = (bz) => Array.from(bz)
...
const hexToBytes = (hex) => {
    if (hex.length % 2 !== 0) {
        return new _1.Result({
            err: new Error(`HexError: hex string must have even length to decode into bytes: hex ${hex}`),
        });
    }
    return _1.Result.ofSafeExec(() => {
        var _a;
        return Uint8Array.from(((_a = hex.match(/.{1,2}/g)) !== null && _a !== void 0 ? _a : []).map((hexTuple) => {
            const byte = parseInt(hexTuple, 16);
            if (Number.isNaN(byte)) {
                throw new Error(`HexError: non-hex characters detected in hex: ${hex}`);
            }
            return byte;
        }));
    });
